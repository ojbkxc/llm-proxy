...
DEFAULT_API_KEY = "sk-wa-fzdxhfghgdfjhfdceac"
...
def _build_codex_config(base_url: str, trusted_projects: Optional[List[str]] = None) -> str:
    ...
    lines = [
        ...
        # effort 固定 high：cfapi 网关不认 ultra/minimal（上游会拒 数据不合法），
        # 桌面版 Codex 可能擅自改回 ultra，重跑部署即纠正回 high。
        'model_reasoning_effort = "high"',
        "disable_response_storage = true",
        "# 权限全放行（管理员级）：永不逐条审批 + 无沙箱，模型命令直接执行，不弹确认框。",
        "approval_policy = \"never\"",
        "# 注意：exec 模式对 sandbox_mode 不生效（0.153.x 已知问题），",
        "# exec 需 `codex exec --sandbox danger-full-access`；桌面版/TUI 读这里的配置。",
...
def _write_codex_config(codex_home: Path, base_url: str, force: bool,
    ...
    if cfg.exists():
        ...
        if "env_key" in text or "model_provider" in text:
            # 无论是否 --force 都整份重建：网关/模型/effort/权限等受管字段强制刷新，
            # 并继承旧配置里已信任的项目，避免丢失。
            inherited = _extract_trusted_projects(text)
            log.info("整份重建 config.toml（强制刷新受管字段；继承信任项目 %d 个）",
                     len(inherited))
    all_projects = list(dict.fromkeys((trusted_projects or []) + inherited))
    _deploy_write(cfg, _build_codex_config(base_url, all_projects))
    return True


def _extract_trusted_projects(text: str) -> List[str]:
    """从现有 config.toml 里提取已信任项目路径（供整份重建时继承）。"""
    out: List[str] = []
    for ln in text.split("\n"):
        m = re.match(r"\s*\[projects\.'([^']+)'\]", ln.strip())
        if m:
